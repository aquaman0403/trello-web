...
function CardUserGroup({ cardMemberIds = [], onUpdateCardMembers }) {
  ...
  // Đoạn này lấy activeBoard từ redux ra để mục đích là lấy được toàn bộ thông tin của những thành viên của board thông qua field FE_allUsers
  const board = useSelector(selectCurrentActiveBoard)
  /**
   * Thành viên trong card sẽ phải là thành viên trong board
   * Vì thế dựa vào mảng board.FE_allUsers card.memberIds rồi chúng ta tạo ra một mảng FE_CardMembers chứa đủ thông tin của user để hiển thị ra ngoài UI, bởi mặc định trong card chỉ lưu id của user
   */
  const FE_CardMembers = board.FE_allUsers?.filter(user => cardMemberIds.includes(user._id))

  const handleUpdateCardMembers = (user) => {
    // console.log(user)
    // Tạo biến incommingMemberInfo để gửi cho BE với 2 thông tin chính là userId và action là xóa hoặc thêm vào card
